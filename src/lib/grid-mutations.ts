import {
  getGrid,
  saveGrid,
  deleteGrid as deleteGridStorage,
  generateId,
  Grid,
} from "./local-storage";
...
export function createGrid(args: {
  ...
  if (
    args.streakMode === "TWICE_WEEKLY" ||
    args.streakMode === "THRICE_WEEKLY"
  ) {
    const g =
      args.streakGoalPerWeek ?? (args.streakMode === "TWICE_WEEKLY" ? 2 : 3);
...
export function toggleDate(args: { gridId: string; isoDate: string }): boolean {
  ...
  // Validate isoDate
  if (!/^\d{4}-\d{2}-\d{2}$/.test(args.isoDate))
...
export function markToday(args: {
  gridId: string;
  useLocal?: boolean;
}): string {
  const grid = getGrid(args.gridId);
...
export function updateGridSettings(args: {
  ...
  if (args.streakGoalPerWeek !== undefined)
    patch.streakGoalPerWeek = args.streakGoalPerWeek;
  if (args.customDaysOfWeek !== undefined)
    patch.customDaysOfWeek = args.customDaysOfWeek;
