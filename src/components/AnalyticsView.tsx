...
export default function AnalyticsView({ grids }: AnalyticsViewProps) {
    const [selectedGridId, setSelectedGridId] = useState<string>(grids[0]?._id || "");
    ...
    const calculateAnalytics = () => {
        ...
        const completedDates = selectedGrid.gridData.completedDates.map((d: string) => new Date(d));
        const datesInRange = completedDates.filter((d: Date) => d >= start && d <= end);
        ...
        const variance = dayOfWeekCounts.reduce((sum, count) => {
        ...
        <div className="max-w-6xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-4 mb-8">
                <div className="flex-1">
                    <label className="block text-sm font-bold mb-2 text-muted-foreground">SELECT GRID</label>
                    <select
                    ...
                    >
                        {grids.map(grid => (
                            <option key={grid._id} value={grid._id}>{grid.title}</option>
                        ))}
                    </select>
                </div>

                <div>
                    <label className="block text-sm font-bold mb-2 text-muted-foreground">TIME PERIOD</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="border-2 border-foreground p-6 bg-background">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-sm font-bold text-muted-foreground uppercase">Completion</h3>
                    </div>
                    <div className="text-4xl font-bold mb-1">{analytics.completionRate}%</div>
                ...
                <div className="border-2 border-foreground p-6 bg-background">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-sm font-bold text-muted-foreground uppercase">Current Streak</h3>
                    </div>
                    <div className="text-4xl font-bold mb-1">{analytics.currentStreak}</div>
                ...
                <div className="border-2 border-foreground p-6 bg-background">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-sm font-bold text-muted-foreground uppercase">Best Streak</h3>
                    </div>
                ...
                <div className="border-2 border-foreground p-6 bg-background">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <h3 className="text-sm font-bold text-muted-foreground uppercase">Total Days</h3>
                    </div>
            ...
            <div className="border-2 border-foreground p-6 mb-8">
                <h3 className="text-sm font-bold mb-4 uppercase tracking-wider">Progress Overview</h3>
            ...
            <div className="border-2 border-foreground p-6 mb-8">
                <h3 className="text-sm font-bold mb-6 uppercase tracking-wider">Day of Week Distribution</h3>
                <div className="grid grid-cols-7 gap-4">
                    {dayNames.map((day, index) => {
                        const count = analytics.dayOfWeekCounts[index];
                        const percentage = analytics.maxDayCount > 0
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="border-2 border-foreground p-6">
                    <h3 className="text-sm font-bold mb-4 uppercase tracking-wider">Consistency Score</h3>
                    <div className="text-6xl font-bold mb-2">{analytics.consistencyScore}</div>
                ...
                <div className="border-2 border-foreground p-6">
                    <h3 className="text-sm font-bold mb-4 uppercase tracking-wider">Streak Mode</h3>
                    <div className="space-y-3">
                        <div>
                            <div className="text-xs text-muted-foreground mb-1">Mode</div>
                            <div className="font-bold">
                                {selectedGrid.streakMode === "EVERY_DAY" && "Every Day"}
                                {selectedGrid.streakMode === "TWICE_DAILY" && "Twice Daily"}
                                {selectedGrid.streakMode === "TIMES_PER_WEEK" && `${selectedGrid.streakGoalPerWeek}x Per Week`}
                                {selectedGrid.streakMode === "EVERY_WEEKDAY" && "Every Weekday"}
                                {selectedGrid.streakMode === "EVERY_OTHER_DAY" && "Every Other Day"}
                                {selectedGrid.streakMode === "WEEKENDS_ONLY" && "Weekends Only"}
                            </div>
                        </div>
                        <div>
